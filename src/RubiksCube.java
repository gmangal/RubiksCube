/*=============================================================================
 |   Assignment:  Program #1:  Rubik's Cube
 |		Program:  RubiksCube.java
 |       Author:  Gaurav Mangal (gmangal@email.arizona.edu)
 |       Grader:  
 |
 |       Course:  CSC 345
 |   Instructor:  Lisa Peairs
 |     Due Date:  July 10, 2013, 11:59 p.m.
 |
 |  Description:  This program implements a 3x3 Rubik's Cube. This program is 
 |				  not a cube solver. It reads an initial state from an input 
 |				  file. The first part of an input file tells the program the 
 |				  initial state of the cube. This is stored in a 3D array of 
 |				  ints. The first dimension of the array represents the 6 
 |				  faces of the cube. The second and third dimensions represent 
 |				  the 9 facelets of each face in a 3x3 array. Following the 
 |				  integers, the file may optionally contain String commands 
 |				  to be interpreted by the program as rotations to the cube. 
 |				  Each command should be on its own line in the input file. 
 |				  These strings are stored in an ArrayList. 
 |
 |				  main uses a switch statement to interpret each String 
 |				  command. Since the commands are read line by line, if there 
 |				  is more than one command on a line, the program will not 
 |				  recognize it as matching a case in the switch statement, 
 |				  and therefore throw an error. 
 |
 |				  After reading the input and performing any specified 
 |				  commands, the program writes the resulting state of the cube 
 |				  to the output file. Both the input and output files are 
 |				  specified by the user as command line arguments.
 |
 |				  First dimension of array represents a specific face. 
 |				  face 0 - Up, face 1 - Down/Bottom, face 2 - Front, 
 |				  face 3 - Back, face 4 - Left, face 5 - Right
 |
 |				  2nd and 3rd dimensions of the array hold the color of the 
 |				  facelet. 
 |				  1 - White, 2 - Yellow, 3 - Red, 
 |				  4 - Orange, 5 - Green, 6 - Blue
 |
 ...
 |				  java.util.Scanner
 | 
 | Instance Vars: cube - 3D array of ints of size [6][3][3]. Represents the 
 |					  Rubik's cube.
 |				  commands - ArrayList of Strings that holds any commands read 
 |					  from the input file. These commands are interpreted as 
 |					  cube rotations by the program.
 |
 | Operational 
 | Requirements:  Program is written in Java and must be compiled using a Java 
 |				  compiler. When run, the program requires two arguments. The 
 |				  first argument is the name of the input file. The filename 
 |				  must exist in the same directory as the java program. The 
 |				  output filename must be provided as the second command line 
 |				  argument. However, the filename need not exist in the 
 |				  current directory. If it does exist, the file will be 
 |				  overwritten by the program. If it does not exist, it will be 
 |				  created and then written to by the program.
 |                
					...
					for (int c = 0; c < 3; c++) {
						switch (cube[a][b][c]) {
							...
							default:
								...
								System.err
										.println("Invalid integer input. There "
												+ "must be exactly 54 integers between 1 "
												+ "and 6 inclusive");
								System.exit(2);
						}
